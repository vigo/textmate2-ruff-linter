...
module Linter
  ...
  def run(options={})
    ...
    if input.nil?
      return TextMate::Process.run(cmd, args, TM_FILEPATH, { :chdir => TM_PROJECT_DIRECTORY })
    else
      return TextMate::Process.run(cmd, args, { :input => input, :chdir => TM_PROJECT_DIRECTORY })
    end
  end
